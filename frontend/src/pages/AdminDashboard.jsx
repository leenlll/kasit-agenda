...
const AdminDashboard = () => {
  ...
  const [pendingDenyEvent, setPendingDenyEvent] = useState(null);
  const fetchCalled = useRef(false);
  const navigate = useNavigate();
  ...
  const handleStatusUpdate = async (eventId, newStatus, organizerEmail, eventName, denialReason = "") => {
    try {
      const eventRef = doc(db, "bookings", eventId);
      const eventDoc = await getDoc(eventRef);
      ...
      await updateDoc(eventRef, {
        status: newStatus,
        ...(newStatus === "Denied" && { denialReason }),
      });

      await sendStatusEmail(organizerEmail, eventName, newStatus, denialReason);

      setEvents((prev) =>
        prev.map((event) =>
          event.id === eventId ? { ...event, status: newStatus, denialReason } : event
        )
      );
    } catch (error) {
      console.error(`❌ Error updating status:`, error);
  ...
  return (
    <div className="admin-page">
      ...
      <Header
        ...
        extraRightContent={
          <div className="header-icons">
            <img src={logIcon} alt="Log" className="log-img" onClick={handleLog} style={{ cursor: "pointer" }} />
            <img src={logoutIcon} alt="Logout" className="logout-img" onClick={handleLogout} style={{ cursor: "pointer" }} />
            <Link to="/">
              <img src={home} alt="Home" className="home-img" />
            </Link>
          </div>
          ...
          <table className="requests-table">
            ...
            <tbody>
              {events.length > 0 ? (
                    ...
                    <td>
                      {event.status === "Pending" ? (
                        <div>
                          <button
                            className="modify-btn"
                            onClick={() => handleStatusUpdate(event.id, "Approved", event.organizerEmail, event.eventName)}
                          >
                            Approve
                          </button>
                          <button
                ...
                <tr><td colSpan="5">No event requests found</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className="modal-overlay">
            <div className="modal-content">
              ...
              {selectedEventStudents.length > 0 ? (
                <ul>
                  {selectedEventStudents.map((student, index) => (
                    <li key={index}>{student.studentName} ({student.email})</li>
                  ))}
                </ul>
              ) : (
                <p>No students registered for this event.</p>
              ...
              <div className="modal-buttons">
                <button className="close-modal-btn" onClick={handleExportPDF}>Download as PDF</button>
          ...
          <div className="modal-overlay">
            <div className="modal-content">
              ...
              <textarea
                ...
                placeholder="Enter reason for denying the event"
                rows={4}
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    handleStatusUpdate(
                      pendingDenyEvent.id,
                      "Denied",
                      pendingDenyEvent.organizerEmail,
                      pendingDenyEvent.eventName,
                      denialReason
                    );
                    setShowReasonModal(false);
                    setDenialReason("");
                    setPendingDenyEvent(null);
                  }}
                >
                  Confirm Denial
                </button>
